            ...
            if value == current_node.value:
                ...
                if parent_node is None:
                    ...
                    if current_node.left is not None and current_node.right is not None:
                        right_subtree_min_value = current_node.right.get_tree_min_value()
                        current_node.value = right_subtree_min_value
                        current_node.right.remove(
                            right_subtree_min_value, current_node)
                ...
                # if the current node has both a left and right node
                # THEN, search the right subtree for the node with the smallest value
                # and update the the current node's value with the right subtree's smallest value
                # and remove the node with the smallest value
                if parent_node is not None and (current_node.left is not None and current_node.right is not None):
                    right_subtree_min_value = current_node.right.get_tree_min_value()
                    current_node.value = right_subtree_min_value
                    current_node.right.remove(
                        right_subtree_min_value, current_node)
                ...
                # if the current node is equal to the parent node's left node
                # THEN, replace the parent node's left node with the current node left node
                elif parent_node is not None and (current_node.left is not None):
                ...
                # if the current node is equal to the parent node's right node
                # THEN, replace the parent node's right node with the current node's right node
                elif parent_node is not None and (current_node.right is not None):
                ...
                # if the current node does NOT have child nodes
                # leaf node
                elif parent_node is not None and (current_node.left is None and current_node.right is None):
                    current_node.value = None

                # found node with the same value as the node to remove value
                # exit the while loop
                break
